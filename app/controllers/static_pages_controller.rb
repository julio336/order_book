...
class StaticPagesController < ApplicationController
  ...
  @bid = Hash.new
    @ask = Hash.new
    url = "https://api.binance.com/api/v3/depth?symbol=ADAUSDT&limit=5000"
    resp = Net::HTTP.get_response(URI.parse(url))
    data = JSON.parse(resp.body)
    data['bids'].each do |bid|
      if bid[1].to_f > 100000
  ...
  url_price = "https://api.binance.com/api/v3/avgPrice?symbol=ADAUSDT"
  resp = Net::HTTP.get_response(URI.parse(url_price))
    @price = JSON.parse(resp.body)
    @price = @price['price'].to_f.round(3)
  #puts data['bids'][0][1] #-> amount of bid
  ...
  def mana
    @bid = Hash.new
      @ask = Hash.new
      url = "https://api.binance.com/api/v3/depth?symbol=MANAUSDT&limit=5000"
      resp = Net::HTTP.get_response(URI.parse(url))
      data = JSON.parse(resp.body)
      data['bids'].each do |bid|
        if bid[1].to_f > 100000
    ...
    url_price = "https://api.binance.com/api/v3/avgPrice?symbol=MANAUSDT"
    resp = Net::HTTP.get_response(URI.parse(url_price))
      @price = JSON.parse(resp.body)
      @price = @price['price'].to_f.round(3)
    #puts data['bids'][0][1] #-> amount of bid
